export default {
  ...
  run: async (client, m) => {
    ...
    const timeSinceLast = now - (user.lastDaily || 0)

    if (timeSinceLast < oneDay) {
      const restante = formatRemainingTime(oneDay - timeSinceLast)
      return await columbina2(client, m, `🌽 Ya has reclamado tu *Daily* de hoy.\n> Puedes reclamarlo de nuevo en *${restante}*`, [], m)
    }
    ...
    if (timeSinceLast > twoDays) {
      ...
      const lossText = `🌱 Has reclamado tu recompensa diaria de *${recompensa.toLocaleString()} ${monedas}*! (Día *1*)\n` +
        `> Día *2* » *¥${siguiente.toLocaleString()}*` +
        (perdioRacha ? `\n> ☆ ¡Has perdido tu racha de días!` : '')
      
      return await columbina2(client, m, lossText, [], m)
    }
    ...
    const claimText = `🌱 Has reclamado tu recompensa diaria de *¥${recompensa.toLocaleString()} ${monedas}* (Día *${user.dailyStreak}*)\n` +
      `> Día *${user.dailyStreak + 1}* » *¥${siguiente.toLocaleString()}*${rachaExtra}`

    await columbina2(client, m, claimText, [], m)
  }
};
