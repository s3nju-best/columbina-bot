export default {
  ...
  run: async (client, m) => {
    ...
    if (chat.adminonly || !chat.rpg) {
      return await columbina2(client, m, `🌱 Estos comandos estan desactivados en este grupo.`)
    }
    ...
    if (remainingTime > 0) {
      return await columbina2(client, m, `🌽 Debes esperar *${msToTime(remainingTime)}* para trabajar de nuevo.`)
    }
    ...
    const mensajeExito = `🌱 ${pickRandom(trabajo)} *¥${rsl.toLocaleString()} ${monedas}*.`;
    
    await columbina2(client, m, mensajeExito)
  }
};
...
const trabajo = [
  ...
  "Eres un organizador de conerencias y recibes",
