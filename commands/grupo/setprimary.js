...
export default {
  ...
  run: async (client, m, args) => {
    try {
      ...
      if (!who2) {
        return await columbina2(client, m, `🍒 Por favor menciona un bot para convertirlo en primario.`, [], m)
      }
      ...
      if (!allowedBots.includes(who)) {
        return await columbina2(client, m, `🌽 El usuario mencionado no es una instancia de Sub-Bot.`, [], m)
      }

      if (!groupParticipants.includes(who)) {
        return await columbina2(client, m, `《✧》 El bot mencionado no está presente en este grupo.`, [], m)
      }

      if (chat.primaryBot === who) {
        return await columbina2(client, m, `🌽 @${who.split('@')[0]} ya es el Bot principal del Grupo.`, [who], m)
      }
